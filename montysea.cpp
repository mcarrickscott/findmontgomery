...
// find a random point on the curve
        ...
        if (Edwards)
		{
			BOOL div4=FALSE;
			BOOL div8=FALSE;
			if (prime(nrp/4)) div4=TRUE;
			if (nrp%8==0 && prime(nrp/8)) div8=TRUE;
			if (div4 || div8)
			{
                Big tt,nrpt;
                BOOL done=FALSE;
				...
				if (div4)
				{
					...
					cout << "NP/8= " << nrp/8 << endl;
					cout << "NP/8 is Prime! " << endl;
				}
                //miracl *mip=&precision;
                //mip->IOBASE=16;
                //cout << "(nrp+nrpt)/4 = " << (nrp+nrpt)/4 << endl;
                //cout << "t mod 3= " << tt%3 << endl;
				...
				if (prime(nrpt/4))
				{
                    cout << "NPT/4= " << nrpt/4 << endl;
					cout << "NPT/4 is Prime! " <<  endl;
                    done=TRUE;
					break;
				}
				if (nrpt%8==0 && prime(nrpt/8))
				{
                    cout << "NPT/8= " << nrpt/8 << endl;
					cout << "NPT/8 is Prime!" << endl;
                    done=TRUE;
					break;
				}
				if (!done)
...
//mip->IOBASE=10;
    ...
    A=((ZZn)3 - MA*MA)/((ZZn)3 * MB*MB);
    B=((ZZn)2 * MA*MA*MA - (ZZn)9 * MA)/((ZZn)27 * MB*MB*MB);
    ...
    ecurve((Big)A,(Big)B,p,MR_AFFINE);
/*
	if (Edwards)
	{
		modulo(p);
...
//
        ...
        { 
            ...
            { // LHS = RHS
                ...
                BOOL done=FALSE;
				if (lp==8 && (p+1-tau)%lp==rmod8)  // or 0
				{
					done=TRUE;
					cout << " ***" << endl;
                    if (search) escape=TRUE;
				}
                if (lp!=8 && (p+1-tau)%lp==0)
                {
                    done=TRUE;
                    cout << " ***" << endl;
...
//cout << "To here 1" << endl;
    if (Edwards)
	{
//cout << "To here 2 " << nrp/4 << endl;
		BOOL div4=FALSE;
		BOOL div8=FALSE;
		if (prime(nrp/4)) div4=TRUE;
		if (nrp%8==0 && prime(nrp/8)) div8=TRUE;
//cout << "rmod8= " << rmod8 << endl;
//if (div4) cout << "div4 is true" << endl;
//if (div8) cout << "div8 is true" << endl;
        if (((rmod8==0 && !div4) || (rmod8==4 && !div8)) && search) {
            do
            {
                a+=4;
            } while (jacobi(a*a-4,p)==1);
...
// double point twice (4*P)
    ...
    cout << "Montgomery Curve y^2=x^3+" << a << "x^2+x" << endl;
    cout << "Edwards Curve x^2+y^2 = 1-" << (a-2)/4 << "x^2.y^2" << endl;
    if (p==nrp) 
    {
